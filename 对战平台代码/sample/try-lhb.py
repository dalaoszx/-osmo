...
def cost_of_theta(relative_i,r1,r2,d0,v0,delta_alpha):
    ...
    t_max=min(int(math.log((r1**2-r2**2)/r1**2,0.99)),30,int(math.sqrt(2*d_alpha/a)))
    T=sympy.symbols('T')
    t=sympy.symbols('t')
...
c2=0
for i in range(20):
    for relative_i in range(0,30):
        time_i,cost_i=cost_of_theta(relative_i,5,3.5,10,0.1,110)
        if cost_i>c:
            c2=relative_i
            c1=time_i
            c=cost_i
